...
import { getAdminTermsOfService, ADMIN_SYNC_EVENT } from '../lib/adminSync';
import Logo from '../components/ui/Logo';

export default function TermsOfService() {
  const navigate = useNavigate();
  const [content, setContent] = useState(getAdminTermsOfService());

  useEffect(() => {
    window.scrollTo(0, 0);
    const handleSync = () => {
      setContent(getAdminTermsOfService());
    };
